...
- 在线时长统计：MongoDB 记录终端上下线时间戳，支持在线时长分析、异常重连追踪与统计报表生成。
...
- 线程池管理：统一配置异步任务线程池，支持动态调整、健康检查与负载监控。
...
### 测试覆盖范围
- **单元测试**：覆盖应用层业务逻辑、领域模型、端口接口实现
- **集成测试**：覆盖基础设施层适配器、WebSocket 连接管理、缓存与事件处理
- **端到端测试**：REST API 端点、WebSocket 协议、RPC 服务调用

### 测试工具与框架
- **JUnit 5** (5.10.0)：单元测试框架
- **Mockito** (5.4.0)：Mock 测试与行为验证
- **Testcontainers** (1.19.0)：容器化集成测试（Redis、MongoDB、MySQL）
- **Awaitility** (4.2.0)：异步测试工具，支持事件驱动场景
- **Spring Boot Test**：集成测试与 MockMvc 测试

### 代码覆盖率目标
- **单元测试覆盖率**：≥80%（关键业务逻辑 ≥90%）
- **集成测试覆盖率**：≥70%（基础设施适配器）
- **分支覆盖率**：≥75%（条件判断与异常处理）

### 质量保障流程
1. 运行 `mvn clean test` 执行单元测试
2. 运行 `mvn clean verify` 生成 JaCoCo 覆盖率报告 (`target/site/jacoco/index.html`)
3. 提交前确保所有测试通过，覆盖率达标
4. 针对新增端点补充契约测试或 MockMvc 测试
5. 使用 `IntegrationTestBase` 封装公共测试配置，确保测试环境一致性
...
#### 🎯 核心功能
- **设备管理**: HTTP/WebSocket 双通道接入，支持终端协议 v1.0/v1.1
- **指令系统**: 命令生成、排队、下发、回执全流程管理
- **状态治理**: 心跳监控、上下线事件、TTL 机制、分布式锁
- **数据存储**: MongoDB 状态报告、MinIO 截图存储、MySQL 账号管理
- **RPC 服务**: Dubbo 服务间通信、Nacos 服务注册与发现
- **可观测性**: Actuator 健康检查、Prometheus 指标、Zipkin 链路追踪

---

## 相关文档
- [Spring Boot 3.3.11 文档](https://docs.spring.io/spring-boot/docs/3.3.11/reference/html/)
- [Dubbo 3.2.11 文档](https://dubbo.apache.org/zh/docs3-v2/)
